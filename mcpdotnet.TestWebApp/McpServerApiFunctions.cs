...
namespace mcpdotnet.TestWebApp;
...
public static class McpApiFunctionExtensions
{
    ...
    public static async Task<WebApplication> UseMcpApiFunctions(this WebApplication app, Assembly? assembly = null)
    {
        ...
        {
            if (!type.IsClass || (!type.Name.EndsWith("Controller") && type.GetCustomAttribute<ApiControllerAttribute>() == null))
                continue;
        ...
        {
            await mcpHostedService.StopAsync(CancellationToken.None);
            ...
            await mcpHostedService.StartAsync(CancellationToken.None);
        }
